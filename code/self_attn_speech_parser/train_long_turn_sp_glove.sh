...
# TRAINING EXAMPLE
MODEL_NAME=turn_long_sp_glove
SEED=0
echo "Training seeding config: " $MODEL_NAME $SEED
python src/main_sparser.py train --use-glove-pretrained --freeze \
    --train-path ${DATA_DIR}/${PREFIX}train.trees\
    --train-sent-id-path ${DATA_DIR}/${PREFIX}train_sent_ids.txt \
    --dev-path ${DATA_DIR}/${PREFIX}dev.trees \
    --dev-sent-id-path ${DATA_DIR}/${PREFIX}dev_sent_ids.txt \
    --prefix ${PREFIX} \
    --feature-path ${FEAT_DIR} \
    --model-path-base ${MODEL_DIR}/${MODEL_NAME} \
    --speech-features duration,pause,partition,pitch,fbank \
    --sentence-max-len 260 \
    --d-model 1536 \
    --d-kv 96 \
    --elmo-dropout 0.3 \
