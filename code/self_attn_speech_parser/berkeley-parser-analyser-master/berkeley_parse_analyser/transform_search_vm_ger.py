...
import sys
from nlp_util import pstree, render_tree, init, treebanks_vm_ger, parse_errors, head_finder, tree_transform
...
def read_tree(text, out_dict, label):
	...
	complete_tree = treebanks_vm_ger.ptb_read_tree(fake_file)
	if complete_tree is None:
		return None
	treebanks_vm_ger.homogenise_tree(complete_tree)
	if not complete_tree.label.strip():
		complete_tree.label = 'ROOT'
	tree = treebanks_vm_ger.apply_collins_rules(complete_tree)
	if tree is None:
...
if __name__ == '__main__':
	from classify_german_vm import classify
