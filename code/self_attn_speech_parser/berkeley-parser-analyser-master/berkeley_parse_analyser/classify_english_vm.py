...
# Table 3.1 stylebook
phrase_labels =  [
	'AP', 'APS', 'ADVP', 'DGP', 'DTP', 'NP', 'NPS', 'PP', 'PPS', 'VP',
	'CNJ', 'CNJ1', 'CNJ2', # conjunctions
	'CMP', 'CMP-WH', # complementizer
	'DT-ART', 'DT-DM', 'DT-QNT', 'DT-WH', 'DT-R', # determiners
	'DG', 'DG-WH', # degree phrases
	'CNUM', 'ONUM', # numbers
	'PR-DM', 'PR-WH', 'PR-R', 'N', # pronouns
	'V-G', 'V-PRP', 'V-PSS', # verbs
	'S', 'SUGG',
	'TOP'
]
...
def classify(info, gold, test):
	...
	if value_present(info, ['label'], ['ADVP', 'AP']):
		info['classified_type'] = "Modifier Attachment"
		return

	if 'subtrees' in info:
		if 'ADVP' in info['subtrees'][1:] or 'AP' in info['subtrees'][1:]:
			info['classified_type'] = "Modifier Attachment"
			return
...
if __name__ == '__main__':
	from transform_search_vm import main
	import sys
