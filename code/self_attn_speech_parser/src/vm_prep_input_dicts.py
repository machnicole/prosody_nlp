...
def make_term2pw_feats(word_times_dir):
    # SWBD xml structure distinguishes between phonwords and terminals
    # but in verbmobil, there are only terminals
    # so: make a dummy term2pw dictionary that is copied from pw_dict
    term2pw = {}
    term2feats = {}
    for picklefile in os.listdir(word_times_dir):
        pw_dict = pickle.load(open(os.path.join(word_times_dir, picklefile), "rb"))

        for pw_id in pw_dict:
            term2pw[pw_id] = pw_id
            term2feats[pw_id] = {'orth': pw_dict[pw_id]['text'],
                                 'start': pw_dict[pw_id]['start_time'],
                                 'end': pw_dict[pw_id]['end_time']}

    return term2pw,term2feats


            
def make_sent2pw_term(term2pw,term2feats):
    # creates two dictionaries: sent2pw and sent2term
    # sent2pw maps from sentence ids to a list of phone word ids
    # sent2term maps from sentence ids to a list of term ids
    # as terminals are the same as phonwords in VM, those dicts are the same here

    sent2pw = {}
    # sent2term = {}

    for pw_id in term2pw:
        sentence_id = pw_id[:-5]
        if sentence_id in sent2pw:
            sent2pw[sentence_id].append(pw_id)
        else:
            sent2pw[sentence_id] = [pw_id]

    sent2term = sent2pw

    return sent2pw,sent2term

def load_pw2feats(sentence_id, alignment_dir,orth2meandur):
    # Load a dict from phonword to features for a given conversation (both speakers)

    pw2feats = defaultdict(dict)
    align_file = f'word_times_{sentence_id}.pickle'
...
def make_sent2pitchpov(sents,sent2pitchpov,sent2times,pitchpov_dir):
...
def make_sent2fbankenergy(sents,sent2fbankenergy,sent2times,fbank_dir): #,sent2turn,turn2sent):

    for sent in sents:
        # spk_sents = [sent for sent in sents if spk in sent]
        fbank_file = f'{sent}.pickle'
...
# Ignore turn related stuff for now:
    ...
    with open(phone2meandur_path,'rb') as f:
        phone2meandur = pickle.load(f, encoding='latin1')
    ...
    for sentence_id in sent_ids:
        pw2feats = load_pw2feats(sentence_id, alignment_dir, orth2meandur)  # pw2feats for just this conversation
        # sents = get_sents_for_conv(conv,sent_ids)
        # sents = discard_no_phone_sents(sents,sent2pw,pw2feats)
        # sents = sorted(sents)
        sent2part,sent2times = make_sent2part_and_times([sentence_id],sent2part,sent2times,sent2term,term2pw,pw2feats,orth2meandur,term2feats,phone2meandur)
        #sent2pause = make_sent2pause(sents,sent2pause,sent2term,term2pw,pw2feats)
        sent2pause = make_reordered_sent2pause([sentence_id],sent2pause,sent2term,term2pw,pw2pause)
        sent2dur = make_sent2dur([sentence_id],sent2dur,sent2part,sent2times,sent2term,term2pw,pw2feats,orth2freq,term2feats,phone2meandur,orth2meandur,sent2rawdur)
        # sent2mfcc = make_sent2mfcc(conv,sents,sent2mfcc,sent2times,mfcc_dir)
        sent2fbankenergy = make_sent2fbankenergy([sentence_id],sent2fbankenergy,sent2times,fbank_dir)#,sent2turn,turn2sent)
        sent2pitchpov = make_sent2pitchpov([sentence_id],sent2pitchpov,sent2times,pitchpov_dir)
