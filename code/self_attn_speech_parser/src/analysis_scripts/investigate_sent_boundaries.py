...
pros_f = os.path.join(outdir,'turn_sp_correct_eval_72240_dev=90.90.pt_dev_predicted.txt')
nonpros_f = os.path.join(outdir,'turn_nonsp_correct_eval_72240_dev=86.09.pt_dev_predicted.txt')
...
for model in ('pros','nonpros'):
    ...
    for tree_id in gold_ids:
        ...
        if not tree_id in oversplit[model] and not tree_id in undersplit[model]:
            ...
            correct_num_sents_correct_split_loc += len(pred_break_idxs.intersection(gold_break_idxs))
            correct_num_sents_incorrect_split_loc += len(pred_break_idxs-gold_break_idxs)
    ...
    total_correct = incorrect_num_sents_correct_split_loc+correct_num_sents_correct_split_loc
    total_incorrect = incorrect_num_sents_incorrect_split_loc+correct_num_sents_incorrect_split_loc

    print('Total num splits:')
    print(f'{model}: {total_correct} correct splits')
    print(f'{model}: {total_incorrect} incorrect splits')

    print(f'total pred splits: {total_pred_splits}')
    print(f'total gold splits: {total_gold_splits}')
    
    precision = total_correct / total_pred_splits
    recall = total_correct / total_gold_splits
