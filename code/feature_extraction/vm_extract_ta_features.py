...
def preprocess_cnn_feats(sentence_id, file_info):
    pitch_pov_file = os.path.join(pitch_pov_dir, \
            '{}.pickle'.format(sentence_id))
...
def extract_features(sentence_id, data_dir, out_dir, dictionaries):
    ...
    raw_cnn_feats = preprocess_cnn_feats(sentence_id, info)
...
if __name__ == '__main__':
    ...
    if args.sentence_id != '0':
        ...
        for in_file in all_files:
            sentence_id = os.path.basename(in_file).split('.')[0][11:]
            print(sentence_id)
            extract_features(sentence_id, data_dir, output_dir, dictionaries)
